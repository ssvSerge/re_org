...
int sha256( const unsigned char *input, size_t ilen,
             unsigned char output[32], int is224 )
{
    //sha256_context ctx;
    sha256_context* ctx = polarssl_malloc(sizeof(sha256_context));
    if (!ctx)
        return POLARSSL_ERR_SHA256_MALLOC_FAILED;   
    sha256_init( ctx );
...
int sha256_hmac_starts( sha256_context *ctx, const unsigned char *key,
...
// ================================================================================================================================
{
    ...
    //unsigned char sum[32];
        ...
        if (keylen < 14)
            return POLARSSL_ERR_SHA256_HMAC_SHORT_KEY;
    #endif
    // JAB
    // ============================================================================================================================
    ...
    if( keylen > 64 )
    {
        ...
        //sha256_free( ctx ); // don't do this because we don't want to erase ctx->scratch
        keylen = ( is224 ) ? 28 : 32;
        key = sum;
        // JAB
        // ========================================================================================================================
...
int sha256_hmac( const unsigned char *key, size_t keylen,
                  ...
                  unsigned char output[32], int is224 )               
{
    ...
    ret = sha256_hmac_starts( ctx, key, keylen, is224 );
    if (ret != 0)
        return ret;
    // JAB
    // ============================================================================================================================
...
int sha256_self_test( int verbose )
{
    ...
    sha256sum = polarssl_malloc(32);
    ctx = polarssl_malloc(sizeof(sha256_context));
    if (!ctx || !buf || !sha256sum)
    {
        if (buf) polarssl_free(buf);
        if (sha256sum) polarssl_free(sha256sum);
    ...
    for( i = 0; i < 14; i++ )
    {
        ...
        if( j == 5 || j == 6 )
        {
            ...
            ret = sha256_hmac_starts( ctx, buf, buflen, k );
            if (ret != 0)   
                goto exit;
            // JAB
            // ====================================================================================================================
        ...
        // JAB
        {
            ...
            ret = sha256_hmac_starts( ctx, sha256_hmac_test_key[j],
