...
{
    //sha1_context ctx;
    sha1_context* ctx = polarssl_malloc(sizeof(sha1_context));
    if (!ctx)
        return POLARSSL_ERR_SHA1_MALLOC_FAILED;
    sha1_init( ctx );
    sha1_starts( ctx );
...
int sha1_hmac_starts( sha1_context *ctx, const unsigned char *key,
// JAB
// ================================================================================================================================
                       size_t keylen )
{
    ...
    unsigned char * sum = ctx->scratch;
        ...
        if (keylen < 14)
            return POLARSSL_ERR_SHA1_HMAC_SHORT_KEY;
    #endif
    // JAB
    // ============================================================================================================================
    ...
    if( keylen > 64 )
    {
        ...
        sha1_finish( ctx, ctx->scratch );
        //sha1_free( ctx ); // don't do this because we don't want to erase ctx->scratch
        // JAB
        // ========================================================================================================================
        keylen = 20;
        key = sum;
...
int sha1_hmac(const unsigned char *key, size_t keylen,
    const unsigned char *input, size_t ilen,
    unsigned char output[20])
{
    int ret;
    ...
    ret = sha1_hmac_starts( ctx, key, keylen );
    if (ret != 0)
        return ret;
    // JAB
    // ============================================================================================================================
...
int sha1_self_test( int verbose )
{
    ...
    for( i = 0; i < 7; i++ )
    {
        ...
        if( i == 5 || i == 6 )
        {
            ...
            ret = sha1_hmac_starts( ctx, buf, buflen );
            if (ret != 0)
                goto exit;
            // JAB
            // ====================================================================================================================
        ...
        else
        {
            ...
            ret = sha1_hmac_starts( ctx, sha1_hmac_test_key[i],
