...
#define SIZEOF_POLARSSL_MPI_BUFFER (POLARSSL_MPI_MAX_SIZE * 2 + 20)
int x509write_crt_set_subject_key_identifier( x509write_cert *ctx )
{
    int ret;
    //unsigned char buf[POLARSSL_MPI_MAX_SIZE * 2 + 20]; /* tag, length + 2xMPI */
    
    unsigned char* buf = polarssl_malloc(POLARSSL_MPI_MAX_SIZE * 2 + 20); /* tag, length + 2xMPI */
    if(buf == NULL ) return POLARSSL_ERR_PK_MALLOC_FAILED;
    
    unsigned char *c = buf + SIZEOF_POLARSSL_MPI_BUFFER;
    size_t len = 0;

    memset( buf, 0, SIZEOF_POLARSSL_MPI_BUFFER );
    ASN1_CHK_ADD( len, pk_write_pubkey( &c, buf, ctx->subject_key ) );

    sha1( buf + SIZEOF_POLARSSL_MPI_BUFFER - len, len, buf + SIZEOF_POLARSSL_MPI_BUFFER - 20 );
    c = buf + SIZEOF_POLARSSL_MPI_BUFFER - 20;
    len = 20;

    ASN1_CHK_ADD( len, asn1_write_len( &c, buf, len ) );
    ASN1_CHK_ADD( len, asn1_write_tag( &c, buf, ASN1_OCTET_STRING ) );

    ret = x509write_crt_set_extension( ctx, OID_SUBJECT_KEY_IDENTIFIER,
                                        OID_SIZE( OID_SUBJECT_KEY_IDENTIFIER ),
                                        0, buf + SIZEOF_POLARSSL_MPI_BUFFER - len, len );
                       ...
                       void *p_rng )
{
    ...
    unsigned char hash[64];
	unsigned char* sig = polarssl_malloc(POLARSSL_MPI_MAX_SIZE);
	if(sig == NULL) return POLARSSL_ERR_PK_MALLOC_FAILED;
	
    unsigned char* tmp_buf = polarssl_malloc(SIZEOF_TMP_BUF);
	if(tmp_buf == NULL )
	{
		polarssl_free(sig);
...
{
    int ret;
    unsigned char*  output_buf = polarssl_malloc(SIZEOF_OUTPUT_BUF*sizeof(unsigned char));
    size_t olen = 0;

    if( ( ret = x509write_crt_der( crt, output_buf, SIZEOF_OUTPUT_BUF*sizeof(unsigned char),
                                   f_rng, p_rng ) ) < 0 )
    {
    ...
    if( ( ret = pem_write_buffer( PEM_BEGIN_CRT, PEM_END_CRT,
                                  output_buf + SIZEOF_OUTPUT_BUF*sizeof(unsigned char) - ret,
                                  ret, buf, size, &olen ) ) != 0 )
    {
    	polarssl_free(output_buf);
        return( ret );
