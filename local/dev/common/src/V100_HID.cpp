...
void Atomic_Hid_Get_Param_Bin::Exec() {
    ...
    if (val_ptr) {
        ...
        JEngineExec::GetInstance()->Jag_Free(val_ptr->data);
...
void Atomic_Hid_Parse_Res_Int::Exec() {
    ...
    V100_ERROR_CODE status;
    m_nRC = GEN_OK;
    status = JEngineExec::GetInstance()->Jag_HFParseResultInt(m_id, &m_val);
    if (GEN_OK != status) {
        m_nRC = status;
    }
    #endif
...
void Atomic_Hid_Parse_Res_Double::Exec() {
    ...
    V100_ERROR_CODE status;
    m_nRC = GEN_OK;
    status = JEngineExec::GetInstance()->Jag_HFParseResultDouble(m_id, &m_val);
    if (GEN_OK != status) {
        m_nRC = status;
    }
    #endif
...
void Atomic_Hid_Parse_Res_Data::Exec() {
    ...
    #if 0
    V100_ERROR_CODE status;
    HFData* data_ptr = nullptr;

    m_nRC = GEN_OK;

    status = JEngineExec::GetInstance()->Jag_HFParseResultData(m_id, &data_ptr);
    ...
    if (data_ptr) {
        _load_array(m_val, data_ptr->data, data_ptr->size);
        HFFree(data_ptr->data);
        HFFree(data_ptr);
    }
    #endif
...
void Atomic_Hid_Parse_Res_Point::Exec() {
    ...
    #if 0
    V100_ERROR_CODE status;
    HFPoint* data_ptr = nullptr;

    m_nRC = GEN_OK;

    status = JEngineExec::GetInstance()->Jag_HFParseResultPoint(m_id, &data_ptr);
...
void Atomic_Hid_Parse_Res_Image::Exec() {
    ...
    #if 0
    V100_ERROR_CODE status;
    HFImage* data_ptr = nullptr;

    m_nRC = GEN_OK;

    status = JEngineExec::GetInstance()->Jag_HFParseResultImage(m_id, &data_ptr);
    ...
    if (data_ptr) {
        m_encoding = data_ptr->imageEncoding;
        _load_array(m_image, data_ptr->data.data, data_ptr->data.size);
        HFFree(data_ptr->data.data);
        HFFree(data_ptr);
    }
    #endif
...
void Atomic_Hid_Parse_Match_Gallery::Exec() {
    ...
    #if 0
    V100_ERROR_CODE status;
    HFMatchGallery* data_ptr = nullptr;
    ...
    status = JEngineExec::GetInstance()->Jag_HFParseResultMatchGallery(m_id, &data_ptr);
...
void Atomic_Hid_Get_Final_Res::GetValue(array_bin_t& val) {
    val = m_data;
}
