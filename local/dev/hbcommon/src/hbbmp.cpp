    ...
    unsigned char blue)
{
   ...
   if (x < 0 || x >= mWidth)
   {
      cout << "Invalid value: " << x << " (expected: 0 <= x < " << mWidth << ")" << endl;
   ...
   if (y < 0 || y >= mHeight)
   {
      cout << "Invalid value: " << y << " (expected: 0 <= y < " << mHeight << ")" << endl;
...
void Bmp::set8BitImage(int x, int y, uint8_t *pImage)
{
   // demosaic first
   std::vector<uint8_t> dem_image;
   dem_image.resize(x * y * 3);
   CreateColorImage(pImage, dem_image.data(), x, y);

   uint8_t *pDst = dem_image.data();

   for (int yy = 0; yy < y; yy++)
   {
      for (int xx = 0; xx < x; xx++)
      {
         mImage[yy][xx] = Color(*(pDst), *(pDst + 1), *(pDst + 2));
         pDst += 3;
      }
   }
}

void Bmp::setTranspose(bool enable)
{
   mTranspose = enable;
}
...
bool Bmp::CreateColorImage(unsigned char *pImage, unsigned char *pColorImage, int nWidth, int nHeight)
{
   bayer_pattern_t type = BAYER_PATTERN_BGGR;
   unsigned char *src = pImage;
   unsigned char *dest = pColorImage;
   int sx = nWidth;
   int sy = nHeight;
