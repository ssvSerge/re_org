...
usb_transport_err_t usb_transport_device_t::io_cancel ( struct iocb* iocb_ptr ) {
    ...
    for ( ; ; ) {
        ...
        // 
        cancel_res = ::io_cancel ( m_io_ctx, iocb_ptr, NULL );
    ...
    if ( cancel_res < 0 ) {
        // Strange. Error returned.
        debug ( "io_cancel() failed; errno=%d; text=%s; (%s):(%d)", errno, strerror(errno), __FUNCTION__, __LINE__ );
        // ret_val = usb_transport_err_t::USB_STATUS_FAILED;
