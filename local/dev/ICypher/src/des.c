                      ...
                      const unsigned char key[DES_KEY_SIZE * 2] )
{
    ...
    // JAB
    //uint32_t sk[96];
    uint32_t * sk = polarssl_malloc(sizeof(uint32_t) * 96);
    if (!sk)
        return POLARSSL_ERR_DES_MALLOC_FAILED;
    // JAB
    // ============================================================================================================================
...
int des_self_test( int verbose )
{
    ...
    // ============================================================================================================================
    // JAB
    des_context * ctx = polarssl_malloc(sizeof(des_context));
    des3_context * ctx3 = polarssl_malloc(sizeof(des3_context));
    unsigned char * tbuf = polarssl_malloc(24);
    if (!ctx || !ctx3 || !tbuf)
        return POLARSSL_ERR_DES_MALLOC_FAILED;
    unsigned char * buf = tbuf;
#if defined(POLARSSL_CIPHER_MODE_CBC)
    unsigned char * prv = tbuf + 8;
    unsigned char * iv  = tbuf + 16;
#endif
    // JAB
    // ============================================================================================================================
