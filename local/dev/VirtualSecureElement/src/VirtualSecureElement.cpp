...
CryptExecStatus VirtualSecureElement::Execute_Encrypt(const uint8_t *in_buffer, uint32_t in_buffer_len, uint8_t **encrypted_out_buffer, uint32_t &encrypted_out_buffer_len, const uint8_t *iv_buffer, uint32_t iv_len)
{
...
CryptExecStatus VirtualSecureElement::Execute_Decrypt(const uint8_t *in_buffer, uint32_t in_buffer_len, uint8_t **decrypted_out_buffer, uint32_t &decrypted_out_buffer_len, const uint8_t *iv_buffer, uint32_t iv_len)
{
    //PrintKey(m_current_key_value, m_current_key_info.key_size);
    if (!in_buffer || !in_buffer_len)
        return CryptExecStatus::Invalid_Argument;
...
CryptExecStatus VirtualSecureElement::Execute_Verify_And_Decrypt(const uint8_t* map_filename, uint32_t map_filename_len, const uint8_t *iv_buffer, uint32_t iv_len)
{
    // Open memory maps.
...
CryptExecStatus VirtualSecureElement::Execute_Hash_Data(const uint8_t *in_buffer, uint32_t in_buffer_len, HashAlgorithms hash_algorithm, uint8_t **out_hash_result, uint32_t &out_hash_result_length)
{
...
CryptExecStatus VirtualSecureElement::Execute_Reset_Secure_Element() {
    ...
    EraseKeySlot(0x1001); // KT_EXT_BSK;
    EraseKeySlot(0x1000); // KT_EXT_DSK;
    EraseKeySlot(22);     // KT_EXTKEY_PRIVATE_EXP;
    EraseKeySlot(21);     // KT_EXTKEY_PUBLIC_EXP;
    EraseKeySlot(20);     // KT_EXTKEY_DEVICE_Q;
    EraseKeySlot(19);     // KT_EXTKEY_DEVICE_P;
    EraseKeySlot(18);     // KT_EXTKEY_DEVICE_PRIVATE;
    EraseKeySlot(17);     // KT_EXTKEY_DEVICE_PUBLIC;
    EraseKeySlot(16);     // KT_EXTKEY_HOST_PUBLIC;
    EraseKeySlot(13);     // KT_EXTKEY_KSN_1;
    EraseKeySlot(12);     // KT_EXTKEY_KSN_0;
    EraseKeySlot(11);     // KT_EXTKEY_AES_VEND;
    EraseKeySlot(10);     // KT_EXTKEY_TDES3;
    EraseKeySlot(9);      // KT_EXTKEY_TDES2);
    EraseKeySlot(8);      // KT_EXTKEY_TDES1;
    EraseKeySlot(7);      // KT_EXTKEY_TDES0;
    EraseKeySlot(6);      // KT_EXTKEY_AES3;
    EraseKeySlot(5);      // KT_EXTKEY_AES2;
    EraseKeySlot(4);      // KT_EXTKEY_AES1;
    EraseKeySlot(3);      // KT_EXTKEY_AES0;
    EraseKeySlot(2);      // KT_EXTKEY_BTK;
    EraseKeySlot(1);      // KT_EXTKEY_CTK;

    ofstream master_key_writer (master_key_filename_path, ios::binary);
    ofstream nFS_writer(rng_fs_path, ios::binary);

    uint8_t  rand_buffer_mk  [32]{};
    uint8_t  rand_buffer_fsk [32]{};
...
CryptExecStatus VirtualSecureElement::Execute_Get_Key_Info(uint32_t key_slot, KeyInfoStructure &out_key_info) {
    ...
    // KT_EXTKEY_PRIVATE_EXP = 22
    if ( key_slot == 19 || key_slot == 20 || key_slot == 22 ) {
        memset ( &out_key_info, 0, sizeof(KeyInfoStructure) );
        #if LOG_ENABLED
            printf ( "%s: slot: %d, err: CryptExecStatus::Not_Supported \n", __FUNCTION__, key_slot );
        #endif
        return CryptExecStatus::Not_Supported;
    }
    ...
    if ( ! IsKeyExist(key_slot) ) {
        memset ( &out_key_info, 0, sizeof(KeyInfoStructure) );
        #if LOG_ENABLED
            printf ( "%s: slot: %d, err: CryptExecStatus::Not_Exist \n", __FUNCTION__, key_slot );
        #endif
        return CryptExecStatus::Not_Exist;
    }
    ...
    if ( get_key_success != CryptExecStatus::Successful ) {
        memset ( &out_key_info, 0, sizeof(KeyInfoStructure) );
        #if LOG_ENABLED
            printf ( "%s: slot: %d, err: Unknown error \n", __FUNCTION__, key_slot );
        #endif
    }
...
CryptExecStatus VirtualSecureElement::Execute_Set_Key(const KeyInfoStructure &key_info, const uint8_t *in_key_buffer, uint32_t in_key_buffer_len) {
    ...
    if (exec_set_successful != CryptExecStatus::Successful) {
        #if LOG_ENABLED
            fprintf(stdout, "SetKey:Setting Key %d failed.", key_info.slot);
        #endif
        return CryptExecStatus::Execute_Error;
    }
...
CryptExecStatus VirtualSecureElement::Execute_Set_Property(const uint8_t *key, uint32_t key_length, const uint8_t *value, uint32_t value_length)
{
    if (!key || !key_length)
        return CryptExecStatus::Invalid_Argument;
...
CryptExecStatus VirtualSecureElement::Execute_Get_Property(const uint8_t *key, uint32_t key_length, uint8_t **value, uint32_t &value_length)
{
    if (!key || !key_length)
        return CryptExecStatus::Invalid_Argument;
    if (!value)
...
bool VirtualSecureElement::GetFSKey(u256 &pKey) {
    ...
    p_AES->Encrypt(rnd_clear_buf, enc_fs_key, sizeof(u256), master_key_buf, sizeof(u256), nullptr, AES_BLOCK_Modes_Type::AES_CBC);
...
bool VirtualSecureElement::Encrypt(u8 *pKey, uint nKeySize, u16 nKeyMode, u8 *pIn, uint nInSize, u8 **pOutCG, uint &nOutCGSize)
{
...
bool VirtualSecureElement::Decrypt(u8 *pKey, uint nKeySize, u16 nKeyMode, u8 *pInCG, uint nInCGSize, u8 *pOut, uint *nOutSize)
{
    ICRYPTOAlgo *pCryptoAlgo = nullptr;
