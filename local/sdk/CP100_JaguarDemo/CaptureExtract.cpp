...
int main()
{
    ...
    VERIFY(HFGetParamString, (HFCONTEXT_NONE, HFPARAM_STRING_VERSION, &version))
    printf("Version - %s\n", version);
    HFFree(version);
    ...
    {
        double quality = 0;
        int32_t err = HFGetIntermediateResult(operation, HFRESULTVALID_BASIC, static_cast<int32_t>(sequenceNumber), &result);
