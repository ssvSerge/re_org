...
int sha512(const unsigned char *input, size_t ilen,
                unsigned char output[64], int is384)
{
   // sha512_context ctx;

    sha512_context *ctx = polarssl_malloc(sizeof(sha512_context));
    if (!ctx)
        return POLARSSL_ERR_SHA512_MALLOC_FAILED;
    sha512_init( ctx );
...
int sha512_hmac_starts( sha512_context *ctx, const unsigned char *key,
...
// ================================================================================================================================
{
    ...
    unsigned char * sum = ctx->scratch;
        ...
        if (keylen < 14)
            return POLARSSL_ERR_SHA512_HMAC_SHORT_KEY;
    #endif
    // JAB
    // ============================================================================================================================
    ...
    if( keylen > 128 )
    {
        ...
        sha512_finish( ctx, sum );
        //sha512_free( ctx ); // don't do this because we don't want to erase ctx->scratch
        keylen = ( is384 ) ? 48 : 64;
        key = sum;
        // JAB
        // ========================================================================================================================
    ...
    polarssl_zeroize( ctx->scratch, sizeof( ctx->scratch ) );
    return 0;
    // JAB
    // ============================================================================================================================
}
...
void sha512_hmac_finish( sha512_context *ctx, unsigned char output[64] )
{
    ...
    unsigned char * tmpbuf = ctx->scratch;
    // JAB
    // ============================================================================================================================
...
//                const unsigned char *input, size_t ilen,
//                unsigned char output[64], int is384 )
int sha512_hmac(const unsigned char *key, size_t keylen,
    const unsigned char *input, size_t ilen,
    unsigned char output[64], int is384)
{
    int ret;
    ...
    if (!ctx)
        return POLARSSL_ERR_SHA512_MALLOC_FAILED;
    sha512_init( ctx );
    ret = sha512_hmac_starts(ctx, key, keylen, is384);
    if (ret != 0)
        return ret;
...
int sha512_self_test( int verbose )
{
    ...
    sha512_context *ctx = NULL;
    buf = polarssl_malloc(1024);
    sha512sum = polarssl_malloc(64);
    ctx = polarssl_malloc(sizeof(sha512_context));
    if (!ctx || !buf || !sha512sum)
    {
        if (buf) polarssl_free(buf);
        if (sha512sum) polarssl_free(sha512sum);
        if (ctx) polarssl_free(ctx);
        return POLARSSL_ERR_SHA512_MALLOC_FAILED;
    }
    // SANG
    // ============================================================================================================================
    ...
    for( i = 0; i < 14; i++ )
    {
        ...
        if( j == 5 || j == 6 )
        {
            ...
            ret = sha512_hmac_starts( ctx, buf, buflen, k );
            if (ret != 0)
                goto exit;
            // JAB
            // ====================================================================================================================
        ...
        // JAB
        {
            ...
            ret = sha512_hmac_starts( ctx, sha512_hmac_test_key[j],
