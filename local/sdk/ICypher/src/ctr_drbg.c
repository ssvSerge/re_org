                   ...
                   size_t entropy_len )
{
    ...
    unsigned char * key = ctx->key_scratch;
    // JAB
    // ============================================================================================================================
...
{
    // ============================================================================================================================
    // JAB - for Blackfin we are forced into a malloc here.  Horrible, but let's just do one of them, not 5.
    ...
    unsigned char *p, *iv;
    //aes_context aes_ctx;
    unsigned char * tbuf = polarssl_malloc(/*buf[*/CTR_DRBG_MAX_SEED_INPUT + CTR_DRBG_BLOCKSIZE + 16/*]*/ +
                                           /*tmp[*/CTR_DRBG_SEEDLEN/*]*/ +
                                           /*key[*/CTR_DRBG_KEYSIZE/*]*/ +
                                           /*chain[*/CTR_DRBG_BLOCKSIZE/*]*/ +
                                           sizeof(aes_context));
    unsigned char * buf     = tbuf;
    unsigned char * tmp     = buf + CTR_DRBG_MAX_SEED_INPUT + CTR_DRBG_BLOCKSIZE + 16;
    unsigned char * key     = tmp + CTR_DRBG_SEEDLEN;
    unsigned char * chain   = key + CTR_DRBG_KEYSIZE;
    aes_context   * aes_ctx = (aes_context *) (chain + CTR_DRBG_BLOCKSIZE);
    int i, j;
    size_t buf_len, use_len;

    if (!tbuf)
        return POLARSSL_ERR_CTR_DRBG_MALLOC_FAILED;
                              ...
                              const unsigned char data[CTR_DRBG_SEEDLEN] )
{
    ...
    // JAB
    //unsigned char tmp[CTR_DRBG_SEEDLEN];
    unsigned char * tmp = ctx->tmp_scratch;
    // JAB
    // ============================================================================================================================
    unsigned char *p = tmp;
    int i, j;
                      ...
                      const unsigned char *additional, size_t add_len )
{
    ...
    unsigned char * add_input = ctx->add_scratch;
    // JAB
    // ============================================================================================================================
                     ...
                     const unsigned char *additional, size_t len )
{
    ...
    unsigned char * seed = ctx->seed_scratch;
    // JAB
    // ============================================================================================================================
    size_t seedlen = 0;
                              ...
                              const unsigned char *additional, size_t add_len )
{
    ...
    // JAB
    //unsigned char add_input[CTR_DRBG_SEEDLEN];
    unsigned char * add_input = ctx->add1_scratch;
    unsigned char *p = output;
    //unsigned char tmp[CTR_DRBG_BLOCKSIZE];
    unsigned char * tmp = ctx->tmp1_scratch;
    // JAB
    // ============================================================================================================================
    int i;
    size_t use_len;
...
// JAB - goto cleanup version...  BlackFin stack requires ctr_drbg_self_test to malloc its context structure...
#define CHK( c )    if( (c) != 0 )                          \
                    {                                       \
                        if( verbose != 0 )                  \
...
{
    // ============================================================================================================================
    // JAB - blackfin requires this to be on heap, as it is rather large...
    //ctr_drbg_context ctx;
    ctr_drbg_context * ctx = polarssl_malloc(sizeof(ctr_drbg_context));
    if (ctx == NULL)
        return POLARSSL_ERR_CTR_DRBG_MALLOC_FAILED;
    // JAB
    // ============================================================================================================================
