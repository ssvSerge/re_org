                       ...
                       void *p_rng )
{
    ...
    unsigned char* hash = polarssl_malloc(64);
    unsigned char* sig  = polarssl_malloc(POLARSSL_MPI_MAX_SIZE);
    unsigned char* tmp_buf = polarssl_malloc(SIZEOF_TMP_BUF);

    if(hash == NULL || sig == NULL || tmp_buf == NULL)
    {
        if(hash) polarssl_free(hash);
        if(sig)  polarssl_free(sig);
        if(tmp_buf) polarssl_free(tmp_buf);
                       ...
                       void *p_rng )
{
    ...
    unsigned char*  output_buf = polarssl_malloc(SIZEOF_OUTPUT_BUF);
    size_t olen = 0;
    if(output_buf == NULL) return -1;
    
    

    if( ( ret = x509write_csr_der( ctx, output_buf, SIZEOF_OUTPUT_BUF,
                                   f_rng, p_rng ) ) < 0 )
    {
    ...
    if( ( ret = pem_write_buffer( PEM_BEGIN_CSR, PEM_END_CSR,
                                  output_buf + SIZEOF_OUTPUT_BUF - ret,
                                  ret, buf, size, &olen ) ) != 0 )
    {
        polarssl_free(output_buf);
        return( ret );
